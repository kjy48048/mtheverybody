    ...
    <div class="row">
      ...
      <div class="col-lg-9">
        <div id="carouselExampleIndicators" class="carousel slide my-4" data-ride="carousel">
          <!-- 내용 넣는 공간 -->
          <form action='${pageContext.request.contextPath }/yummyboard/actionUpdateYummyContentForm?yum_idx=${yumVO.yum_idx }' role="form" method="post" class="writeForm" autocomplete="off">
              <div class="form-group">
                  <label for="content-title" class="sr-only">제목</label>
                  <input type="text" name="yum_title" class="form-control" value='${yumVO.yum_title }' placeholder="제목을 입력해 주세요" required>
              </div>
              <div class="form-group">
                  <label for="content-option" class="sr-only">글 구분</label>
                  <select name="yum_category" class="form-control">
				<!-- 관리자인 경우 공지 추가 -->
				<c:if test='${yumVO.yum_category=="공지" }'>
					<option value='공지' selected='selected'>공지</option>
				</c:if>
				
				<c:choose>
					<c:when test='${yumVO.yum_category=="레시피" }'>
						<option value='레시피' selected='selected'>레시피</option>
						<option value='정보'>정보</option>
						<option value='후기'>후기</option>
					</c:when>
					<c:when test='${yumVO.yum_category=="정보" }'>
						<option value='레시피'>레시피</option>
						<option value='정보' selected='selected'>정보</option>
						<option value='후기'>후기</option>
					</c:when>
					<c:when test='${yumVO.yum_category=="후기" }'>
						<option value='레시피'>레시피</option>
						<option value='정보'>정보</option>
            ...
            <!-- 글 입력 -->
            <textarea name="yum_content" id="editor1" rows="20" cols="80" required>${yumVO.yum_content }</textarea>
            <script>
